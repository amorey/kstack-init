...
`scripts/install.sh` is the source for the `curl … | bash` bootstrap hosted at `https://kubestack.xyz/install.sh`. A verbatim copy lives in the `kubetail-website` repo's static assets and is **not automatically synced** — when you edit this file, copy it over manually.
